	...
	var Core = function() {
		return {
			...
			viewport: function() {
				...
				document.documentElement.style.setProperty('--perimtr-viewport-width', viewport.width);
				document.documentElement.style.setProperty('--perimtr-viewport-height', viewport.height);
				document.documentElement.style.setProperty('--perimtr-body-height', vars.elBody.clientHeight);
	...
	return {
		...
		Menus: function() {
			...
			var _hover = function() {
				...
				var menuHoverDelay = getComputedStyle(__core.getVars.elHeader).getPropertyValue('--perimtr-primary-menu-submenu-display-speed') || 666;
			...
			var _fullWidth = function() {
				...
				if( __core.getVars.elHeader.classList.contains('floating-header') ) {
					var floatingHeaderPadding = getComputedStyle(document.querySelector('#header')).getPropertyValue('--perimtr-header-floating-padding');
					document.querySelectorAll('.mega-menu:not(.mega-menu-full):not(.mega-menu-small) .mega-menu-content').forEach( function(el) {
			...
			return {
				init: function(selector) {
					...
					if( __core.getVars.elHeader.classList.contains('no-sticky') ) {
						...
						headerHeight = getComputedStyle(__core.getVars.elHeader).getPropertyValue('--perimtr-header-height').split('px')[0];
					} else {
						headerHeight = getComputedStyle(__core.getVars.elHeader).getPropertyValue('--perimtr-header-height-shrink').split('px')[0];
					}

					pageMenu.style.setProperty("--perimtr-page-submenu-sticky-offset", headerHeight+'px');
				...
				init: function(selector) {
					...
					if( elLoaderColor ) {
						if( elLoaderColor == 'theme' ) {
							elLoaderCSSVar = ' style="--perimtr-loader-color:var(--perimtr-themecolor);"';
						} else {
							elLoaderCSSVar = ' style="--perimtr-loader-color:'+elLoaderColor+';"';
						}
					}
					...
					if( elSpeedIn ) {
						__core.getVars.elWrapper.style.setProperty('--perimtr-animate-duration', Number(elSpeedIn)+'ms');
						if( elAnimIn == 'fadeIn' ) {
							pageTransition.style.setProperty('--perimtr-animate-duration', Number(elSpeedIn)+'ms');
						}
					}
					...
					var endPageTransition = function() {
						...
						var displayContent = function() {
							...
							setTimeout(function() {
								__core.getVars.elWrapper.style.removeProperty('--perimtr-animate-duration');
							}, 666);
						};
				...
				init: function(selector) {
					...
					__core.isFuncTrue( function() {
						...
						selector.forEach( function(element) {
							...
							elBar.style.setProperty( '--perimtr-progress-speed', elSpeed );
				...
				init: function(selector) {
					...
					__core.isFuncTrue( function() {
						...
						var element = selector,
							...
							elMsg = element.attr('data-notify-msg') || 'Thank you for contacting us. We will get back to you soon!',
		...
		TextSplitter: function() {
			...
			var _splitter = function(el, type = 'word') {
				...
				el.querySelectorAll('span')?.forEach( function(elem, index) {
					elem.style.setProperty('--perimtr-split-index', index + 1);
				});
			};
			...
			var _percent = function(params) {
				...
				var onScroll = function(params) {
					fastdom.measure( function(){
						...
						params.elem.style.setProperty('--perimtr-scroll-percent', percent);
						params.elem.style.setProperty('--perimtr-scroll-ratio', ratio);
						params.elem.style.setProperty('--perimtr-scroll-start', start);
						params.elem.style.setProperty('--perimtr-scroll-end', end);
					});
				};
						...
						var initParams = function(selector) {
							...
							selector.forEach( function(elem) {
								...
								if( includeWidth == 'true' || (elem.classList.contains('parallax') && elem.getAttribute('data-parallax-direction') == 'horizontal') ) {
									elem.style.setProperty('--perimtr-scroll-width', params.width);
								}

								if( includeHeight == 'true' || (elem.classList.contains('parallax') && elem.getAttribute('data-parallax-direction') != 'horizontal') ) {
									elem.style.setProperty('--perimtr-scroll-height', params.height);
								}

								if( includeOffset == 'true' ) {
									elem.style.setProperty('--perimtr-scroll-offset', params.offset);
								}

								if( !isNaN(parallaxRatio) ) {
									elem.style.setProperty('--perimtr-parallax-ratio', parallaxRatio);
								}
